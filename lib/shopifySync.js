...
totalPriceSet { shopMoney { amount } }
customer { displayName }
fulfillmentOrders(first: 5) { edges { node { status deliveryMethod { methodType } } } }
}
}
}
}`,
...
const onlineOrders = unfulfilled
.filter((o) => {
const fos = (o.fulfillmentOrders?.edges || []).map((fe) => fe.node);
const readyForPickup = fos.some((fo) => fo.deliveryMethod?.methodType === "PICK_UP" && fo.status === "IN_PROGRESS");
