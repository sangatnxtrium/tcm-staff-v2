...
{ label: "TCG", collectionId: "gid://shopify/Collection/485149901079", productType: "Non-Sports Card" },
...
const lotTotals = {};
const mLots = (await getModule("merchandise_lots")) || [];
for (const lot of mLots) {
if (!Array.isArray(lot.countedOrderIds)) lot.countedOrderIds = [];
if (typeof lot.soldCountTotal !== "number") lot.soldCountTotal = 0;
if (typeof lot.soldRevenueTotal !== "number") lot.soldRevenueTotal = 0;
}
const mLotsById = new Map(mLots.map((l) => [l.id, l]));
...
const mLot = mLotsById.get(lotId);
if (mLot && !mLot.countedOrderIds.includes(o.id)) {
mLot.countedOrderIds.push(o.id);
mLot.soldCountTotal += 1;
mLot.soldRevenueTotal += amount;
}
}
}
if (inLast30) {
...
lot.soldRevenueTotal = Math.round((lot.soldRevenueTotal || 0) * 100) / 100;
lot.profitTotal = Math.round(((lot.soldRevenueTotal || 0) - (lot.purchasePrice || 0)) * 100) / 100;
lot.itemsRemaining = Math.max(0, (lot.itemCount || 1) - (lot.soldCountTotal || 0));
if (lot.stage === "Listed" && !lot.archived && (lot.soldCountTotal || 0) > 0 && lot.itemsRemaining === 0) {
lot.archived = true;
lot.archivedAt = new Date().toISOString();
}
}
...
staffError = staffErr.message || String(staffErr);
console.error("[staff-sales-sync-error]", staffError);
}
...
await setModule("sync_status", { asOf: new Date().toISOString(), ok: true, staffAttributionAvailable: staffAvailable, staffError: staffError || null });
