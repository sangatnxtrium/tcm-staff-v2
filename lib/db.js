...
`)}export async function getUserByEmail(e){return(await(s`SELECT * FROM users WHERE email = ${e} LIMIT 1;`))[0]||null}export async function getModule(e){const a=await(s`SELECT data FROM modules WHERE name = ${e} LIMIT 1;`);return a[0]?a[0].data:null}export async function setModule(e,a){await(s`
