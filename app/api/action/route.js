...
import{probeDiscogsCollection}from"../../../lib/discogsImport";function c(){return(new Date).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}
                                                                                 )}
...
export async function POST(l){const h=e().get(n)?.value,g=await a(h);if(!g)return t.json({error:"Not signed in"}
                              ...
                              =y||{};
                                ...
                                case"syncNow":{try{const result=await runSyncNow();return t.json({ok:!0,data:result})}catch(syncErr){return t.json({error:syncErr.message||"Sync failed"},{status:500})}}case"discogsProbe":{try{const result=await probeDiscogsCollection();return t.json({ok:!0,data:result})}catch(probeErr){return t.json({error:probeErr.message||"Discogs probe failed"},{status:500})}}
