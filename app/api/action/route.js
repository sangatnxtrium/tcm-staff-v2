import{NextResponse as t}
  from"next/server";import{cookies as e}
    ...
    from"../../../lib/seedData";import{runSync as runSyncNow}from"../../../lib/shopifySync";import{runVinylEnrichBatch}from"../../../lib/vinylEnrich";function c(){return(new Date).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}
                                                                                 )}
...
export async function POST(l){const h=e().get(n)?.value,g=await a(h);if(!g)return t.json({error:"Not signed in"}
                              ...
                              =y||{};
                                ...
                                "`}
                                  ...
                                  case"addBuyingLead":{const{title:e,customer:a,value:n,closer:r}
                                                       =y;if(!e)return t.json({error:"Title is required"}
                                                                              ,{status:400}
                                                                                                    ...
                                                                                                    case"deleteBuyingLead":{const{id:e}=y,a=await o("buying_leads")||[],n=a.filter(t=>t.id!==e);return await i("buying_leads",n),t.json({ok:!0,data:n})}case"editBuyingLeadPricing":{const{id:e,purchasePrice:a,pricedAt:n}=y,r=await o("buying_leads")||[],l=r.find(t=>t.id===e);if(!l)return t.json({error:"Lead not found"},{status:404});return void 0!==a&&(l.purchasePrice=Number(a)||0),void 0!==n&&(l.pricedAt=Number(n)||0),await i("buying_leads",r),t.json({ok:!0,data:r})}case"deleteCollection":{const{index:e}=y,a=await o("collections_profit")||[];if(!(e>=0)||e>=a.length)return t.json({error:"Not found"},{status:404});return a.splice(e,1),await i("collections_profit",a),t.json({ok:!0,data:a})}case"addMerchandiseLot":{const{title:e,source:a,estValue:n,itemCount:r,closer:l,purchasePrice:pp,notes:nt}=y;if(!e)return t.json({error:"Title is required"},{status:400});const c=await o("merchandise_lots")||[],h=c.length?Math.max(...c.map(t=>t.id))+1:1,w=(new Date).toISOString();return c.push({id:h,title:e,source:a||"—",lotTag:"LOT-"+h,stage:L[0],estValue:Number(n)||0,purchasePrice:Number(pp)||0,itemCount:Number(r)||1,assignedTo:f,closer:l||f,notes:nt||"",stageHistory:[{stage:L[0],enteredAt:w,byUser:g.email}]}),await i("merchandise_lots",c),t.json({ok:!0,data:c})}case"moveMerchandiseLot":{const{id:e,stage:a}=y,n=await o("merchandise_lots")||[],r=n.find(t=>t.id===e);if(!r)return t.json({error:"Lot not found"},{status:404});r.stage=a,r.stageHistory=r.stageHistory||[],r.stageHistory.push({stage:a,enteredAt:(new Date).toISOString(),byUser:g.email});return await i("merchandise_lots",n),t.json({ok:!0,data:n})}case"migrateLegacyLots":{const e={"Lead Received":"Sourcing","Appointment Scheduled":"Sourcing","Offer Made":"Sourcing","Purchased":"Purchased","Transport":"Receiving","Processing":"Processing","Grading":"Processing","Listing":"Listed","Sold":"Listed","Checked In":"Receiving","Sorted":"Processing","Graded":"Processing","Priced":"Inventory","Photographed":"Inventory","Listed Online":"Listed","Moved to Floor":"Listed"},a=await o("buying_leads")||[],n=await o("inventory_items")||[],r=(new Date).toISOString(),l=a.map(t=>({id:t.id,title:t.title,source:t.customer||"—",lotTag:"LOT-"+t.id,stage:e[t.stage]||"Sourcing",estValue:t.value||0,purchasePrice:t.purchasePrice||0,itemCount:1,assignedTo:t.closer||"—",closer:t.closer||"—",notes:"Migrated from Buying Pipeline",stageHistory:[{stage:e[t.stage]||"Sourcing",enteredAt:r,byUser:"migration"}]})),c=n.map(t=>({id:100000+t.id,title:t.title,source:t.source||"—",lotTag:"LOT-"+(100000+t.id),stage:e[t.stage]||"Receiving",estValue:0,purchasePrice:0,itemCount:t.items||1,assignedTo:"—",closer:"—",notes:"Migrated from Inventory Workflow",stageHistory:[{stage:e[t.stage]||"Receiving",enteredAt:r,byUser:"migration"}]})),h=[...l,...c];return await i("merchandise_lots",h),t.json({ok:!0,data:h,migratedLeads:l.length,migratedItems:c.length})}case"deleteMerchandiseLot":{const{id:e}=y,a=await o("merchandise_lots")||[],n=a.filter(t=>t.id!==e);return await i("merchandise_lots",n),t.json({ok:!0,data:n})}case"editMerchandiseLotPricing":{const{id:e,purchasePrice:a,estValue:n,itemCount:ic,closer:cl}=y,r=await o("merchandise_lots")||[],l=r.find(t=>t.id===e);if(!l)return t.json({error:"Lot not found"},{status:404});return void 0!==a&&(l.purchasePrice=Number(a)||0),void 0!==n&&(l.estValue=Number(n)||0),void 0!==ic&&(l.itemCount=Number(ic)||1),cl&&(l.closer=cl),await i("merchandise_lots",r),t.json({ok:!0,data:r})}
                                  ...
                                  case"updateProjectStatus":{const{projectId:e,status:a}=y,n=await o("projects")||[],r=n.find(t=>t.id===e);if(!r)return t.json({error:"Project not found"},{status:404});if(!s.includes(f)&&f!==r.owner)return t.json({error:"Only the project owner, Operations Manager, or Owner can update this"},{status:403});if(!["Planning","In Progress","On Hold","Done"].includes(a))return t.json({error:"Invalid status"},{status:400});return r.status=a,await i("projects",n),t.json({ok:!0,data:n})}
                                ...
                                case"syncNow":{try{const result=await runSyncNow();return t.json({ok:!0,data:result})}catch(syncErr){return t.json({error:syncErr.message||"Sync failed"},{status:500})}}case"enrichVinylBatch":{try{const result=await runVinylEnrichBatch();return t.json({ok:!0,data:result})}catch(enrichErr){return t.json({error:enrichErr.message||"Enrichment failed"},{status:500})}}case"deleteSprint":{const{id:e}=y,a=await o("sprints")||[],n=a.filter(t=>t.id!==e);return await i("sprints",n),t.json({ok:!0,data:n})}default:return t.json({error:"Unknown action type"}
                                                      ,{status:400}
                                                      )}
                                                                   }
